...
loadarch () {
	...
	if [ "$ARM_V9A" -eq 1 ]; then
		...
		export CFLAGS="-march=armv9-a+sve2+sve2-bitperm+sme+sha3+sm4+lse+dotprod -mtune=cortex-x3 -O3 -flto=thin -ffast-math -fno-math-errno -fomit-frame-pointer -fdata-sections -ffunction-sections -funroll-loops -fno-strict-aliasing"
		export CXXFLAGS="$CFLAGS"
		export LDFLAGS="$LDFLAGS -flto=thin -fuse-ld=lld -Wl,--gc-sections"
		...
		export CFLAGS="-march=armv8-a+crypto+crc -mtune=cortex-a76 -O3 -flto=thin -ffast-math -fno-math-errno -fomit-frame-pointer -fdata-sections -ffunction-sections -funroll-loops -fno-strict-aliasing"
		export CXXFLAGS="$CFLAGS"
		export LDFLAGS="$LDFLAGS -flto=thin -fuse-ld=lld -Wl,--gc-sections"
	fi
